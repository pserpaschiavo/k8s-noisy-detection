                    ...
                    if group_by_phase:
                        if 'phase' in group_df_round.columns:
                            ...
                            for phase_name, group_df_phase in group_df_round.groupby('phase'):
                                # Check if the metric is one that needs conversion from Bytes to MB
                                ...
                                phases_data_for_round[phase_name] = group_df_phase.copy()
                        ...
                        # Check if the metric is one that needs conversion from Bytes to MB
                        metrics_to_convert_to_mb = ['memory_usage', 'disk_throughput_total', 'network_total_bandwidth']
                        if metric in metrics_to_convert_to_mb and 'value' in group_df_round.columns:
                            group_df_round['value'] = group_df_round['value'] / (1024 * 1024)  # Convert Bytes to Megabytes
                            print(f"    Converted metric '{metric}' from Bytes to Megabytes.")
                        rounds_data_for_metric[round_name] = group_df_round.copy()
            else:
