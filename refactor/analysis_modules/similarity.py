...
def calculate_pairwise_cosine_similarity(data_df: pd.DataFrame, time_col: str, metric_col: str, group_col: str, min_observations: int = 10):
    ...
    for group1, group2 in combinations(group_ids, 2):
        ...
        if min_len == 0:
            ...
            try:
                # Check if either series is constant (would result in division by zero)
                if np.all(series1_trimmed == series1_trimmed[0]) or np.all(series2_trimmed == series2_trimmed[0]):
                    cosine_val = np.nan
                else:
                    # Calculate cosine similarity: dot(a, b) / (||a|| * ||b||)
                    dot_product = np.dot(series1_trimmed, series2_trimmed)
                    norm_a = np.linalg.norm(series1_trimmed)
                    norm_b = np.linalg.norm(series2_trimmed)
                    cosine_val = dot_product / (norm_a * norm_b)
            except Exception as e:
                print(f"Error calculating cosine similarity between {group1} and {group2} for metric '{metric_col}': {e}")
                cosine_val = np.nan
        
        # Assign similarity value to both positions in the matrix (it's symmetric)
